...
void CtrlStepMotor::RequestKFState()
{
    // Request KF-estimated velocity, acceleration, and current from motor driver
    // CAN mode 0x1B: KF state request (motor replies with vel/acc/current)
    uint8_t mode = 0x1B;
    txHeader.StdId = nodeID << 7 | mode;
    txHeader.DLC = 0; // No payload, just a request

    CanSendMessage(get_can_ctx(hcan), canBuf, &txHeader);
    txHeader.DLC = 8; // Restore default
}


void CtrlStepMotor::UpdateKFStateCallback(float _vel, float _acc, float _current)
{
    // Called from CAN RX ISR when motor driver responds with KF state
    // Velocity is in motor-side steps/s, convert to output deg/s
    float sign = inverseDirection ? -1.0f : 1.0f;
    float stepsPerDeg = (float)(CTRL_CIRCLE_COUNT) / 360.0f * (float)reduction;

    estVelocity = sign * _vel / stepsPerDeg;       // deg/s at output
    estAcceleration = sign * _acc / stepsPerDeg;    // deg/s^2 at output
    motorCurrent = _current;                         // Amperes
}
