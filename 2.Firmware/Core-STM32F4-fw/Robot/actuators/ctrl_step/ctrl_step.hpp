...
public:
    ...
    // Phase 4: KF-estimated state feedback from motor driver
    float estVelocity = 0;      // KF-estimated velocity (deg/s at output shaft)
    float estAcceleration = 0;  // KF-estimated acceleration (deg/s^2)
    float motorCurrent = 0;     // Measured motor current (A)
    ...
    // Phase 4: Request and receive KF state from motor driver via CAN
    void RequestKFState();
    void UpdateKFStateCallback(float _vel, float _acc, float _current);
