...
void ThreadControlLoopFixUpdate(void* argument)
{
    for (;;)
    {
        ...
        if (dummy.IsEnabled())
        {
            ...
            {
                case DevClawRobot::COMMAND_TARGET_POINT_SEQUENTIAL:
                case DevClawRobot::COMMAND_TARGET_POINT_INTERRUPTABLE:
                case DevClawRobot::COMMAND_CONTINUES_TRAJECTORY:
                    dummy.MoveJoints(dummy.targetJoints);
                    dummy.UpdateJointPose6D();
                    break;
                case DevClawRobot::COMMAND_MOTOR_TUNING:
