...
void Motor::CloseLoopControlTick()
{
    ...
    if (useKalmanFilter)
    {
        // Phase 2: Kalman Filter state estimation
        // Provides optimal fusion of encoder measurement and motion model
        // Outputs: position, velocity, AND acceleration (bonus: needed for DOB)
        JointKalmanFilter::State_t kfState = kalmanFilter.Update(
            (float)controller->realPosition, 0.0f);

        controller->estVelocity = (int32_t)kfState.velocity;
        controller->estLeadPosition = Controller::CompensateAdvancedAngle(controller->estVelocity);
        controller->estPosition = (int32_t)kfState.position + controller->estLeadPosition;
    }
    else
    {
        // Legacy IIR velocity estimator (kept as fallback)
